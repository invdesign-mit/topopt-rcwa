...
//find the k vectors
int Lattice_Reciprocate(const double *Lr, double *Lk){
  double d;
...
int Lattice_SetKs(const int nG, const double kx0, const double ky0, const int *G, double *Lk, double *kx, double *ky){
  
  for(int i = 0; i < nG; ++i){
    kx[i] = kx0 + 2*M_PI*(Lk[0]*G[2*i+0] + Lk[2]*G[2*i+1]);
    ky[i] = ky0 + 2*M_PI*(Lk[1]*G[2*i+0] + Lk[3]*G[2*i+1]);
  }

  return 0;
}

void MultSV(const int nG, const std::complex<double> *S, const std::complex<double> *input, std::complex<double> *output){
  RNP::TBLAS::MultMV<'N'>(4*nG, 4*nG, std::complex<double>(1,0), S, 4*nG, input, 1, std::complex<double>(0,0), output, 1);
}
/////
...
void TranslateAmplitudes2(
	...
	for(size_t i = 0; i < n2; ++i){
		...
		// against overflow. This doesn't solve the problem completely, but it helps.
		if(0. != ab[i]){
		  ab[i]    *= std::exp(iq*dz_a);
		}
		if(0. != ab[i+n2]){
		  ab[i+n2] *= std::exp(iq*dz_b);
		}
	}
}
...
void GetFieldOnGrid(
	...
	for(size_t i = 0; i < n; ++i){
		...
		if(
		        ( -int(nxy[0])/2 < iu && iu <= int(nxy[0])/2 ) &&
			( -int(nxy[1])/2 < iv && iv <= int(nxy[1])/2 ) 
		){
